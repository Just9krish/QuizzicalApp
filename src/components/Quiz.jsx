export default function Quiz(props) {
    ...
    const quiz = props.allQuiz.map((quiz, id) => {
        ...
        return (<div key={id} className="text-clr-blue-text space-y-5">
            <p className="text-lg font-karla font-bold">{quiz.question}</p>
            <div className="flex flex-wrap gap-5">
                {optionsArr.map((option, id) =>
                    <button key={id} className="border border-clr-blue-btn rounded-md py-1 px-6 font-inter font-medium hover:bg-clr-choose hover:border hover:border-clr-choose">
                        {option}</button>
                )}
            </div>
